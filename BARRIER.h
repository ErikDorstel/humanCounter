...
void setBarrier(char value,String UID) {
...
void barrierWorker() {
  ...
  if (barrierAold!=barrierAstate && (!barrierAstate) && millis()>=barrierA2timer) {
    if (millis()>=barrierA1timer) { alarmOn(); int x1=getData("00000000")+1; setData(x1,"00000000"); }
    else { barrierA1timer=millis()+barrier1Delay; greenOn(barrier1Delay); int x2=getData(UIDA)+1;
      if (x2>4) { x2=4; barrierA1timer=0; greenOff(); alarmOn(); int x3=getData("00000000")+1; setData(x3,"00000000"); }
      setData(x2,UIDA); } }
  ...
  if (barrierBold!=barrierBstate && (!barrierBstate) && millis()>=barrierB2timer) {
    if (millis()>=barrierB1timer) { alarmOn(); int y1=getData("00000000")-1; setData(y1,"00000000"); }
    else { barrierB1timer=millis()+barrier1Delay; greenOn(barrier1Delay); int y2=getData(UIDB)-1;
      if (y2<0) { y2=0; barrierB1timer=0; greenOff(); alarmOn(); int y3=getData("00000000")-1; setData(y3,"00000000"); }
